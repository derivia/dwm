...
void swallow(Client *p, Client *c)
{
  ...
  if (c->noswallow && !swallowfloating && c->isfloating) return;
...
void buttonpress(XEvent *e)
{
  ...
  if (ev->window == selmon->barwin) {
    ...
    if (i < LENGTH(tags)) {
      click = ClkTagBar;
      arg.ui = 1 << i;
    } else if (ev->x < x + TEXTW(selmon->ltsymbol)) click = ClkLtSymbol;
...
void clientmessage(XEvent *e)
{
  ...
  if (cme->message_type == netatom[NetWMState]) {
    ...
    if (cme->data.l[1] == netatom[NetWMSticky] ||
      ...
      setsticky(
          c, (cme->data.l[0] == 1 || (cme->data.l[0] == 2 && !c->issticky)));
...
void destroynotify(XEvent *e)
{
  ...
  else if ((c = swallowingclient(ev->window))) unmanage(c->swallowing, 1);
}
...
pid_t getstatusbarpid()
{
  ...
  if (statuspid > 0) {
    ...
    if ((fp = fopen(buf, "r"))) {
      ...
      while ((c = strchr(str, '/'))) str = c + 1;
      fclose(fp);
      if (!strcmp(str, STATUSBAR)) return statuspid;
    }
  }
  if (!(fp = popen("pidof -s " STATUSBAR, "r"))) return -1;
  fgets(buf, sizeof(buf), fp);
  pclose(fp);
...
void sigstatusbar(const Arg *arg)
{
  ...
  if (!statussig) return;
  sv.sival_int = arg->i;
  if ((statuspid = getstatusbarpid()) <= 0) return;
